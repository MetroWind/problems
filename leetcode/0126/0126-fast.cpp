...
public:
    ...
    PathsType findPaths(std::list<std::vector<std::string>>& paths,
                        const std::string& endWord, std::list<string>& word_list)
    {
        ...
        for(auto iPath = std::begin(paths); iPath != std::end(paths); ++iPath)
        {
            // cout << "hhh" << endl;
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList)
    {
        ...
        std::list<std::vector<std::string>> Paths{{beginWord}};
...
{
    Solution Sol;
    vector<string> Dict{"hot", "dog"};
    Sol.findLadders("hot", "dog", Dict);
    return 0;
}
