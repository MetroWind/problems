...
public:
    ...
    int findPaths(PathsType& paths, const std::string& endWord,
                  std::list<string>& word_list)
    {
        // std::list<std::vector<std::string>> NewPaths;
        std::vector<typename std::list<string>::iterator> Locs;
        if(paths.size() == 0)
        {
            return 0;
        }
        // cout << "Working on " << paths.size() << " paths with length " << paths.front().size() << endl;
        ...
        {
            // paths.pop_front();
            // cout << "lll" << endl; printSeq(Path);

            auto& Begin = Path.back();
            if(canTransform(Begin, endWord))
            {
                // cout << "iii" << endl;
                return Path.size() + 1;
            }
            ...
            for(auto i = std::begin(word_list); i != std::end(word_list); ++i)
            {
                auto ThePath = Path;
                // cout << "fff" << endl;
                if(canTransform(Begin, *i))
                {
                    // cout << Begin << " >> " << *i << "!" << endl;

                    if(std::find(std::begin(Locs), std::end(Locs), i) == std::end(Locs))
                    {
                        Locs.push_back(i);
                    }

                    // cout << "eee" << endl;
                    NewPaths.push_back(ThePath);
                    // cout << "ddd" << endl;
                    NewPaths.back().push_back(*i);
                    // cout << "New path: "; printSeq(NewPaths.back());
                }
            }
            // paths.erase(iPath);
        }
        ...
        if(word_list.size() > 0)
        {
            return findPaths(NewPaths, endWord, word_list);
        }
        else
        {
            return 0;
    ...
    int ladderLength(string beginWord, string endWord, vector<string>& wordList)
    {
        ...
        if(std::find(std::begin(wordList), std::end(wordList), beginWord) != std::end(wordList))
        {
            return 0;
        }

        if(std::find(std::begin(wordList), std::end(wordList), endWord) == std::end(wordList))
        {
            return 0;
        }
        ...
        PathsType Paths{{beginWord}};
        std::list<std::string> Words(std::begin(wordList), std::end(wordList));
        return findPaths(Paths, endWord, Words);
    }
};
...
{
    Solution Sol;
    vector<string> Dict{"hot","cog","dog","tot","hog","hop","pot","dot"};
